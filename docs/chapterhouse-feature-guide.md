...
#### Deep Research Mode (Phase 27E)

Instead of pasting URLs yourself, you can give Intel a **topic** — and it will do the research for you. Type a topic like "homeschool regulation changes 2026" and Intel:
1. Searches the web via Tavily for relevant URLs
2. Discovers the most relevant sources
3. Runs them through the full two-pass Intel analysis pipeline
4. Creates a complete Intel report labeled "Deep Research — [your topic]"

This is for when you know WHAT you want to research but don't have URLs yet.

#### Intel → Task Bridge (Phase 23C)

After an Intel report is complete, you can turn any finding into a task with one click. Each actionable finding becomes a task with `source_type='intel'` so you can trace it back to the original report.
...
**One sentence:** One-click generation of 21 specific document types — each with its own structure, format, and rules — using full Scott context automatically injected.
...
#### Document Export (Phase 20A)

After generating any document, you can **export it** as:
- **DOCX** — Microsoft Word format, ready to email or share
- **Markdown** — Clean `.md` file for developers or documentation
- **PDF** — Print-ready format

#### Outline-First Authoring (Phase 21A)

Instead of generating a full document immediately, you can generate an **outline first**. The AI proposes section headings and a brief description of what each section will cover. You review the outline, approve or tweak it, then tell the AI to expand each section into full content. Like approving a blueprint before building the house.

#### Agentic Editing (Phase 21B)

After a document is generated, you can give the AI targeted editing instructions — "make the intro more punchy" or "rewrite section 3 for beginners." The AI reads the entire document, identifies just the section you mean, and rewrites only that section. Everything else stays untouched.

#### Voice Analysis (Phase 21C)

AI reads your finished document and scores it against your selected brand voice on five dimensions: tone, formality, conviction, clarity, and brand alignment (each 0–10). It also gives specific suggestions for improvement. Like getting editor's notes from someone who knows your brand.

#### Listen Button (Phase 24A/B)

Any document gets a **Listen** button. Click it and ElevenLabs reads the document out loud in a premium voice. Requires `ELEVENLABS_API_KEY` and `ELEVENLABS_DEFAULT_VOICE_ID` environment variables.

#### Brand Voice Injection (Phase 20C)

When generating a document, you can select one of your saved brand voices (from Settings). The AI adapts its writing style to match that voice — warm and teacher-like for NCHO, confident and secular for SomersSchool, etc.

#### Audience Targeting (Phase 28E)

When generating a document, you can select a target audience (from the Audiences page). The AI tailors the content to that audience's demographics, pain points, motivations, and preferred tone.

#### Multi-Surface Output (Phase 28C)

When generating a document, you can request additional output surfaces:
- **DOCX** — automatic Word file export
- **Email summary** — a condensed version suitable for email (generated by Haiku)
- **Social set** — social media posts derived from the document content

These are generated automatically after the main document streams.

---

#### All 21 document types (in plain English)
...
**16. Campaign Plan**
Creates a full multi-phase marketing campaign. Fill in: campaign name, goal, brand (NCHO / SomersSchool / Alana Terry), budget range, and timeline. The output is a phased campaign plan with audience targeting, channel strategy, messaging per phase, KPIs, and a go/no-go checklist. Follows the "convicted-not-curious" brand principle.

> Best for: Planning a product launch, seasonal promotion, or brand awareness campaign.

---

**17. Email Sequence**
Generates a multi-email drip sequence. Fill in: sequence goal, brand, number of emails (3, 5, 7, or 10), and the offer or topic. Each email includes a subject line, body copy, and CTA (call to action). The sequence follows a logical progression — awareness → interest → decision → action. Brand voice is enforced per brand.

> Best for: Welcome sequences, launch sequences, nurture campaigns, re-engagement flows.

---

**18. SEO Audit**
Analyzes a page or topic for search engine optimization. Fill in: the URL or topic, target keywords, and which brand. The output: current strengths, weaknesses, keyword gap analysis, on-page optimization suggestions, content structure recommendations, and a prioritized action list. Every recommendation is specific and actionable — not generic advice.

> Best for: Improving NCHO product pages, SomersSchool landing pages, or blog post SEO.

---

**19. Competitive Brief**
A structured competitive analysis document. Fill in: competitor name, their product/service, your product, and what you want to understand. The output: competitor strengths, weaknesses, pricing analysis, market positioning, your differentiation, and recommended counter-positioning. More focused than the Competitive Positioning doc type (#8) — this is a full brief, not a positioning statement.

> Best for: Deep competitor analysis before a launch or pricing decision.

---

**20. Status Report**
The smartest doc type — it **auto-fetches live data** before generating. When you create a Status Report, the AI automatically pulls:
- Last 7 Folio entries (your intelligence narrative)
- Active Dreams (your idea pipeline)
- Completed Tasks (what got done)
- Published Social posts (your content output)

The output: an executive-style status report covering progress, blockers, upcoming priorities, and recommendations. No manual data gathering needed.

> Best for: Weekly status updates; investor updates; personal accountability check-ins.

---

**21. Feature Spec**
A detailed technical feature specification. Fill in: feature name, problem statement, target user, proposed solution, and any constraints. The output: a multi-section spec with user stories, acceptance criteria, technical approach, data model changes, API surface, edge cases, and testing strategy. Uses the "Decision / Why / Not" format for any architectural choices.

> Best for: Before building any new feature in Chapterhouse or SomersSchool.
...
#### Prediction Scoring (Phase 25A)

Before you approve a post, AI predicts how well it will perform. Claude Haiku scores each post on reach, clicks, and engagement (0–100 scale) based on brand, platform, and content analysis. Weak posts are flagged so you can boost or rewrite them before publishing.

#### Social Boost (Phase 25B)

Got a post with a low prediction score? Hit **Boost** and Claude Sonnet rewrites it for higher engagement — punchier hook, stronger CTA, better hashtags. The original text is preserved in edit history so you can compare before/after.

#### Grounded Mode (Phase 25C)

When generating social posts, flip on **Grounded Mode**. This forces the AI to only make claims backed by your actual research items and knowledge library entries. No ungrounded claims, no invented statistics, no generic marketing fluff. Every assertion has a source.
...
Configuration and targeting pages.

---

### Audiences (`/audiences`)

**One sentence:** Define WHO you're writing for — then select an audience when generating documents so the AI tailors everything to them.

#### What an audience contains

Each audience persona has:
- **Name** — e.g., "Conviction-Stage Homeschool Parent"
- **Description** — Who is this person? What stage are they at?
- **Demographics** — Age ranges, location type, tech comfort, decision stage
- **Pain Points** — What keeps them up at night? (one per line, stored as an array)
- **Motivations** — Why are they doing this? What do they hope for? (one per line, stored as an array)
- **Preferred Tone** — How should content sound when written for this audience?
- **Linked Brand Voice** — Optional connection to a brand voice from Settings

#### How audiences work with Doc Studio

When you generate any document in Doc Studio, you can select a target audience. The AI receives the full audience context — pain points, motivations, demographics, tone — and tailors the document accordingly. A Status Report for an investor reads very differently than one for a homeschool parent.

#### Seed audience

A seed audience is pre-loaded: **"Conviction-Stage Homeschool Parent"** — a parent who has already decided to homeschool and is looking for the right curriculum, not permission to homeschool.

#### CRUD

Full create, edit, and delete from the page. Gold/amber styling consistent with the rest of Chapterhouse.
...
#### Section 2: Deferred Feature Placeholders

Three cards show features that are planned but intentionally not built yet:

| Card | Status | What it will be |
|---|---|---|
| **MCP Server (27C)** | Planned (blue) | Expose Council, Intel, Dreams, and Folio as MCP tools for Claude Code. Waiting for stateless MCP spec (Q2/Q3 2026). |
| **Meta-Agent / EVE (27D)** | Planned (blue) | Agent that creates specialized sub-agents from Council knowledge. Requires Composable Workflows. |
| **Workflow Triggers (28B)** | Foundation Built (amber) | Cron and webhook triggers for workflows. Manual trigger available now at /api/workflows/run. |

#### Section 3: Brand Voices

Manage voice definitions for each brand. Editable inline. These are injected into social media generation and Doc Studio when a brand voice is selected.

#### Section 4: Character Library

Manage AI image generation characters (like Gimli). Physical descriptions, art styles, reference images, and LoRA model IDs.

#### Section 5: Founder Memory
...
| Claude Haiku 4.5 | Email triage and categorization, prompt enhancement (character picker), Intel fact-check pass, voice analysis, prediction scoring, row-level enrichment, knowledge ingest, email summaries (multi-surface) |
...
### API-Only Features (No Dedicated Page)

These features were built in the Cowork Upgrade (Phases 20–28) and are available via API but do not have their own page in the sidebar:

| Feature | API Route | What it does |
|---|---|---|
| **Council Quick-Consult** | `/api/council/quick` | Ask all 5 Council members one question, get 5 answers back instantly — no background job |
| **Knowledge Ingest** | `/api/knowledge/ingest` | Give it a URL → fetches → AI extracts knowledge → saves to Knowledge Library |
| **Watch URLs** | `/api/watch-urls` | Monitor URLs for content changes; auto-ingest when something changes |
| **Row-Level Enrichment** | `/api/enrich/[table]` | AI batch-enriches Dreams (priority+effort), Intel (business track), Social (tone+alt text) |
| **Composable Workflows** | `/api/workflows` + `/api/workflows/run` | Chain AI steps together: council query → doc generate → social posts. Manual trigger only for now. |
| **Cost Tracking** | `/api/costs/summary` | Langfuse API query showing AI spend (requires Langfuse env vars) |
| **GEO Visibility** | `/api/cron/geo-visibility` | Weekly: asks AI search engines your brand queries, checks if you're mentioned |
...
*Document generated: based on code examination of `navigation.ts`, `doc-type-prompts.ts`, `personas.ts`, `knowledge/page.tsx`, `blog/page.tsx`, `unread-triage/page.tsx`, `review-queue/page.tsx`, `audiences/page.tsx`, and supporting component files. Updated April 13, 2026 — Cowork Upgrade Spec (Phases 20A–28E): 6 new doc types (21 total), Audiences page, document export/outline/editing/voice-analysis/listen, social prediction/boost/grounded mode, Intel deep research, knowledge ingest, watch URLs, enrichment, workflows, multi-surface output, GEO visibility, and deferred phase placeholders.*
