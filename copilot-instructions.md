...
| reference/customer-avatar.md | Full customer avatar for NCHO — "Alaskan Annie." Demographics, psychographics, A/B test results (names, colors, slogans, offers), visual identity, brand voice, positioning pillars. |
...
| reference/api-guide-master.md | Master API guide — all 30+ services with live keys, use cases, and commercial status. Local only. NEVER commit. |
...
| docs/workbench/jobs-test-prompts.md | Ready-to-paste test prompts for the curriculum factory pipeline — all grade levels and subjects, covers all 6 passes. |
| intel/social-media-automation-brain.md | Social Media Automation spec — Buffer GraphQL API details, 3-brand voice rules, post lifecycle, analytics pull-back. |
...
6. **Shopify API section in `reference/api-guide-master.md`** — ⏳ API access working (Yellow CoPilot app). Still need `SHOPIFY_WEBHOOK_SECRET` for order webhook verification before launch. Add full entry there.
7. ~~**Helicone**~~ — DEAD. Acquired by Mintlify March 3, 2026. Maintenance mode. Replaced by Langfuse.
8. ~~**Langfuse**~~ — DONE ✅. Keys in hand (see `reference/api-guide-master.md` and .env.master). Wire into all AI-calling apps before charging customers.
