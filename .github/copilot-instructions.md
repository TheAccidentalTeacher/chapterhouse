...
| chapterhouse | TS + Python | 🔴 Active | Private ops brain — **Phases 1–18 + Cowork Upgrade (20A–28E) ALL COMPLETE.** 6-pass Council (Gandalf→Data→Polgara→Earl on GPT-5.4→Silk on gpt-5-mini), jobs (QStash→Railway), n8n, social media automation (Buffer GraphQL), YouTube Intelligence (Gemini 2.5 Flash on Railway), email AI pipeline, **Brand Voices DB** (migration 023), **Character Library** (Gimli LoRA + Replicate img2img, migration 024), **Course Asset Dashboard** (CoursePlatform dual-Supabase bridge, `COURSE_SUPABASE_URL`/`COURSE_SUPABASE_SERVICE_ROLE_KEY`), **Chat Metaknowledge** (migration 029 — keyword-triggered live queries, `APP_ARCHITECTURE_BLOCK`), **Doc Studio** (migration 030 — 21 doc types, SSE streaming, `/doc-studio`), **The Folio** (migration 031 — narrative+track_signals JSONB, daily 05:00 UTC rebuild, `/folio`), jobs RLS anon SELECT fix (migration 032), **Focus Board + Scratchpad** (migrations 039–040), **Blog Pipeline** (migration 041), **Knowledge Library** (migration 044 — knowledge_nodes table, `/knowledge` page), **Task Subtasks + Schema** (migrations 042–043), **Cowork Upgrade** (migrations 100–107 — document export, outline-first authoring, agentic editing, voice analysis, ElevenLabs TTS, council quick-consult, knowledge ingest, watch URLs, intel task bridge, social prediction/boost/grounded mode, row-level enrichment, composable workflows, GEO visibility, target audiences `/audiences`). All tables: `briefs, research_items, opportunities, tasks, chat_threads, knowledge_summaries, knowledge_nodes, founder_notes, jobs, social_accounts, social_posts, emails, context_files, dreams, dream_log, intel_sessions, intel_categories, generated_images, brand_voices, characters, documents, folio_entries, focus_items, scratch_notes, blog_posts, watch_urls, workflows, target_audiences`. Last migration: 107. **📌 PINNED — NOT YET BUILT: Chat-Initiated Document Export** — `documents` table + `html-to-docx` installed; chat route needs to call `/api/documents/generate/` and return doc ID + download URL inline. Use cases: Session Close from chat, spec .md export, Council output as .docx. **Deferred:** 27C MCP Server (Q2/Q3 2026), 27D Meta-Agent/EVE (requires 28B Advanced), 28B Advanced Workflow Triggers (manual trigger built). |
...
| `intel/scott-brain-chapterhouse-handoff.md` | Complete integration spec for Chapterhouse `/api/brain/sync` — full TypeScript route code, env var table, inject_order values for `context_files`. Silk migration checklist for 3 Chapterhouse files (B&B→Silk completed March 28). |
...
3. **Shopify API section in `reference/api-guide-master.md`** — ⏳ Still needs `SHOPIFY_WEBHOOK_SECRET` for order webhook verification.
...
5. ❗️~~**Execute Chapterhouse Phase 0**~~ — **DONE. Phases 1–18 + Cowork Upgrade (20A–28E) ALL COMPLETE (April 13, 2026). Last migration: 107.** Built all 18 core phases + 21 Cowork Upgrade phases in a single overnight session (13 commits, 8 migrations). Includes: document export, outline-first authoring, agentic editing, voice analysis, ElevenLabs TTS, council quick-consult, knowledge ingest, watch URLs, intel task bridge, social prediction/boost/grounded mode, 6 new doc types (21 total), row-level enrichment, composable workflows, GEO visibility, target audiences. Deferred: 27C MCP, 27D Meta-Agent, 28B Advanced triggers. Current build focus: Chat-Initiated Document Export (PINNED, not yet built).
...
13. **Langfuse** — ✅ Wired into Chapterhouse (Phase 20B, `/api/costs/summary` route + `src/lib/langfuse.ts` traceAI wrapper). Env vars: `LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL` set in Vercel. Still need to wire into SomersSchool + BibleSaaS before charging customers.
...
- Client Secret: **REDACTED — see .env.local or `reference/api-guide-master.md`**
...
**April 13–14, 2026 (Cowork Upgrade Spec + Documentation Audit) — Phases 20A–28E built in a single overnight session + full documentation audit.** 13 commits, 8 migrations (100–107). Replaces Cowork subscription. New capabilities: document export (DOCX/Markdown/PDF), outline-first authoring + versioning, agentic editing (Claude Sonnet targeted section rewrites), voice analysis (Haiku 4.5 brand scoring), ElevenLabs TTS in Doc Studio, council quick-consult (synchronous all-5-member), knowledge ingest (URL→Tavily→Haiku→knowledge_nodes), watch URLs (change detection + auto-ingest), intel→task bridge, intel deep research (topic discovery), social prediction scoring + boost + grounded mode, 6 new doc types (21 total: campaign_plan, email_sequence, seo_audit, competitive_brief, status_report, feature_spec), row-level enrichment (dreams/intel/social), composable workflows (sequential step executor), multi-surface output (docx + email_summary + social_set), GEO/AI search visibility tracking, target audiences with doc generation injection. New tables: `watch_urls`, `workflows`, `target_audiences`. New env vars: `LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`, `ELEVENLABS_API_KEY`, `ELEVENLABS_DEFAULT_VOICE_ID`, `GROQ_API_KEY` (optional). `/audiences` page with full CRUD UI. Unified search expanded to 9 tables. Deferred: 27C MCP Server (Q2/Q3 2026), 27D Meta-Agent/EVE, 28B Advanced workflow triggers. Last migration: 107. All documentation (CLAUDE.md, copilot-instructions.md, chapterhouse-feature-guide.md) audited and updated.
...
**March 14, 2026 (Session 6) — Council of the Unserious complete overhaul + national standards alignment.** Fellowship Council replaced by Council of the Unserious across all three systems. 5-pass pipeline: Gandalf → Data → Polgara → Earl → Silk.
