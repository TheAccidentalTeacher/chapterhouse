...
| chapterhouse | TS + Python | 🔴 Active | Private ops brain — **Phases 1–18 ALL COMPLETE.** 6-pass Council (Gandalf→Data→Polgara→Earl on GPT-5.4→Silk on gpt-5-mini), jobs (QStash→Railway), n8n, social media automation (Buffer GraphQL), YouTube Intelligence (Gemini 2.5 Flash on Railway), email AI pipeline, **Brand Voices DB** (migration 023), **Character Library** (Gimli LoRA + Replicate img2img, migration 024), **Course Asset Dashboard** (CoursePlatform dual-Supabase bridge, `COURSE_SUPABASE_URL`/`COURSE_SUPABASE_SERVICE_ROLE_KEY`), **Chat Metaknowledge** (migration 029 — keyword-triggered live queries, `APP_ARCHITECTURE_BLOCK`), **Doc Studio** (migration 030 — 15 doc types, SSE streaming, `/doc-studio`), **The Folio** (migration 031 — narrative+track_signals JSONB, daily 05:00 UTC rebuild, `/folio`), jobs RLS anon SELECT fix (migration 032), **Focus Board + Scratchpad** (migrations 039–040), **Blog Pipeline** (migration 041), **Knowledge Library** (migration 044 — knowledge_nodes table, `/knowledge` page), **Task Subtasks + Schema** (migrations 042–043). All tables: `briefs, research_items, opportunities, tasks, chat_threads, knowledge_summaries, knowledge_nodes, founder_notes, jobs, social_accounts, social_posts, emails, context_files, dreams, dream_log, intel_sessions, intel_categories, generated_images, brand_voices, characters, documents, folio_entries, focus_items, scratch_notes, blog_posts`. Last migration: 044. **📌 PINNED — NOT YET BUILT: Chat-Initiated Document Export** — `documents` table + `html-to-docx` installed; chat route needs to call `/api/documents/generate/` and return doc ID + download URL inline. Use cases: Session Close from chat, spec .md export, Council output as .docx. |
...
**April 14, 2026 (Documentation Audit) — CLAUDE.md + copilot-instructions.md brought fully up to date.** Code-first audit of all documentation. Phases 15–18 added to all docs (Focus Board, Blog Pipeline, Knowledge Library, Task Subtasks). 15th document type (Academic Comparison Paper) documented. `knowledge_nodes` table added to all table references. Last migration: 044. All 18 Chapterhouse phases now complete.
